...
class Metrics:
  ...
  def count_vague_words(self,text):
    ...
    return count
  def check_data_sources(self,text):
    source_indicators=['according to', 'reported', 'data from',
                      'source:', 'as of', 'latest data']
    return sum(phrase in text.lower() for phrase in source_indicators)

  def calculate_reliability_score(self,text):
    """ Calculating reliability based on various factors"""

    #Positive Indicators
    source_citations=self.check_data_sources(text)
    #Negative Indicators
    vague_words = self.count_vague_words(text)
    hedging_phrases = sum(phrase in text.lower() for phrase in
                             ['approximately', 'around', 'roughly', 'estimated'])


    reliability = (source_citations * 20) - (vague_words * 10) - (hedging_phrases * 5)
  ...
  def analyze_response(self,query,system_type,response,time_taken):
    result={
        ...
        'source_citations': self.check_data_sources(response),
    ...
    print(f"   Source citations: {result['source_citations']}")
  ...
  def comparison(self):
    ...
    dual_reliability = sum(r['reliability_score'] for r in dual) / len(dual)
    single_reliability = sum(r['reliability_score'] for r in single) / len(single)
    ...
    print(f"\nUncertain Language (Lower = Better):")
    print(f"   Dual-Agent:   {dual_uncertain:.1f}")
    print(f"   Single-Agent: {single_uncertain:.1f}")
    ...
    print(f"   Dual-Agent:   {dual_reliability:.1f}/100")
    if single_reliability>0:
      print(f"   Single-Agent: {single_reliability:.1f}/100")
    else:
      print(f"0")

    if dual_reliability > single_reliability:
      improvement = ((dual_reliability - single_reliability) / single_reliability * 100)
      print(f"   Improvement: +{improvement:.1f}%")
...
def financial_query_analysis_for_dual(query):
  ...
  try:
    ...
    """
    research_data = researcher_agent.invoke({"input": researcher_agent_prompt})["output"]
...
# Using the Analyst agent
  ...
  try:
    ...
    Here is the data:
    {research_data}
    Create a factual report following these guidelines:
    1. State specific facts directly without adding uncertainty
    2. When presenting data, use phrases like "according to the data", "reports indicate", "data shows"
    3. Only use uncertain language ("may", "could") when the research data itself is uncertain
...
def single_agent_analysis(query):
   ...
   try:
    ...
    - Conclusion
    """
    report = single_agent.invoke({"input": prompt})["output"]
    end_time=time.time()
